...
begin
    ...
    /*여러개의 조건을 사용할 경우Java와 같이 else if를 사용하지 않고
    elsif로 기술해야한다. 또한 중괄호 대신 then과 end if;가 사용된다.*/
    if emp_dept = 50 then
        dept_name := 'Shipping';
...
begin
    ...
    loop
        ...
        /*증가연산자나 복합대입연산자가 없으므로 일반적인 방식으로 변수를 증가시켜야한다.*/
        num := num + 1;
        /*num이 10을 초과하면 loop문을 탈출한다. exit는 Java의 break와 동일하게
        반복문을 탈출한다.*/
        exit when (num > 10);
    end loop;
end;
...
declare
    ...
    변수명을 사용하지만 Oracle에서는 그룹함수 sum()이 있으므로 아래와 같이
    다른 이름을 사용해야한다.*/
    sumNum number := 0;
begin
    loop 
...
커서(cursor)
    : select 문장에 의해 여러행이 반환되는 경우 각 행에 접근하기 위한 개체
        ...
        open cursor
        : 쿼리를 수행하라는 의미. 즉 Open할 때 Cursor선언시의 select문장이 실행되어
...
declare
    --부서테이블의 전체 컬럼을 참조하는 참조변수로 선언한다.
    v_dept departments%rowtype;
    /*커서 선언 : 부서테이블의 모든 레코드를 조회하는 select문으로 into절이
                없는 형태로 쿼리문을 작성한다. 쿼리의 실행결과가 cur1에 저장된다.*/
    cursor cur1 is
        select 
...
begin
    ...
    open cur1;
    --basic루푸문으로 얻어온 결과셋의 갯수만큼 반복하여 인출한다.
    loop
