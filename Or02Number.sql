...
설명 : 숫자데이터를 처리하기 위한 숫자관련 함수를 알아보자
    테이블 생성시 number 타입으로 선언된 컬럼에 저장된 데이터를 대상으로 본다.
*********************/
--현재 접속한 계정에 생성된 테이블, 뷰의 목록을 보여준다. 
select * from tab;
--이와같이 테이블이 없는 경우에는 쿼리에러가 발생한다.
select * from tjoen;--에러
...
select abs(salary) "급여의절대값" from employees order by  salary;
...
trunc() : 소수점을 특정자리수에서 잘라낼 때 사용하는 함수
    형식 : trunc(컬럼명 혹은 값, 소수점이하 자리수)
        두번째 인자가 
            ...
            없을 떄 : 정수부만 표현, 즉 소수점 아래부분은 버림
            음수일 때 : 정수브를 숫자만큼 잘라 나머지를 0으로 채움
*/
...
--1. 영섭사원을 찾아 인출한다.(영업사원의 job_id는 SA_XX로 되어있다.)
...
소수점 관련함수
    ...
    두번째 인자가 
        ...
        있는 경우 : 숫자만큼 소수점이 표현되므로 그 다음수가 5이상이면
                    올림, 미만이면 벌림
*/
select ceil(32.8) from dual;--무조건 올림 33
select ceil(32.2) from dual;--무조건 올림 33

select floor(32.8) from dual;--무조건 버림 32
select floor(32.2) from dual;--무조건 버림 32

select round(0.123), round(0.543) from dual;--버림과 올림을 하여 0, 1이 인출됨
/*
