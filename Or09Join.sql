...
파일명 : Or09Join.sql
테이블 조인
설명 : 두개 이상의 테이블을 동시에 참조하여 데이터를 가져와야 할 때 사용하는 sql문
****************/
...
/*
1] inner join(내부조인)
-가장 많이 사용하는 조인문으로 테이블간에 연결조건을 모두 만족하는 레코드를 검색할 사용한다.
-일반적으로 기본키(Primary key)와 외래키(foreign key)를 사용하여 join하는 경우가 대부분이다.
-두개의 테이블에 동일한 이름의 컬럼이 존자하면 "테이블명.컬럼명" 형태로 기술해야한다.
...
*/
select
    employee_id, first_name, last_name, email, /*이부분에서 에러발생*/department_id, department_name
from employees inner join departments
    on employees.department_id = departments.department_id;
    
--열의 정의가 애매한 경우 컬럼 앞에 테이블명을 추가하면 된다.
select
    employee_id, first_name, last_name, email, departments.department_id, department_name
from employees inner join departments
    on employees.department_id = departments.department_id;
    
--as(알리아스)를 통해 테이블에 별칭을 부여하면 간소화 할 수 있다.  
select
...
즉, inner join은 테이블 양쪽 모두 만족되는 레코드가 인출하게 된다. 
*/    
--3개 이상의 테이블 조인하기
/*
...
--5.join 쿼리문 작성
/*양쪽 테이블에 동시에 존재하는 컬럼인 경우에는 반드시 테이블명이나 별칭을 명시해야 한다.*/
select
    first_name, last_name, email,
    employees.department_id, department_name,
    city, state_province, employees.job_id, job_title
from locations
...
--테이블의 별칭을 사용하면 쿼리문을 조금 간다낳게 만들 수 있다. 
select
    first_name, last_name, email,
...
형식2] 오라클 방식
    ...
    from 테이블1, 테이블2
    where
        테이블1.기본키컬럼 = 테이블2.외래키컬럼 and 조건1 and 조건2....;
표준방식에서 사용한 inner join과 on을 제거하고 조인의 조건을 where절에 표기하는 방식이다.
*/    
...
select
    first_name, last_name, email, D.department_id, department_name, j.job_id, job_title, city
...
/*
2] outer join(외부조인)
outer join은 inner join과 달리 두 테이블에 조인조건이 정확히 일치하지 않아도  기준이 되는
테이블에서 레코드를 인출하는 방식이다. outer join을 사용할 때는 반드시 기준이 되는 테이블을 결정하고 쿼리문을
작성해야한다.
...
*/
/*
시나리오] 전체직원의 사원번호, 이름, 부서아이디, 부서명, 지역을 외부조인(left)을 통해 출력하시오.
*/
select 
...
/*실행결과를 보면 내부조인과는 다르게 107개가 인출된다. 부서가 지정되지 않은 사원까지 인출되기 때문인데
...
형식2(오라클방식)
    ...
    where
        테이블1,기본키 = 테이블2.외래키(+)
        and 조건1 or 조건2;
==> 오라클 방식으로 변경시는 outer join 연산자인 (+)를 붙여준다.
...
--여기 설명 못 적음.
select
    employee_id, first_name, last_name, nvl(department_name, '<부서없음>')department
from employees Em
    left outer join departments De
        on Em.department_id = De.department_id
where to_char(hire_date, 'yyyy') = '2007';        
...
where
 ...
 /*
 3] self join(셀프조인)
 셀프조인은 하나의 테이블에 있는 컬럼끼리 연결해야하는 경우 사용한다.
 즉, 자기자신의 테이블과 조인을 맺는것이다. 셀프조인에서는 별칭이 테이블을 구분하는 구분자의
 역할을 하므로 굉장히 중요하다.
 형식] select 별칭1.컬럼, 컬럼2.컬럼..
        from 테이블A 별칭1, 테이블A 별칭2
        where 별칭1.컬럼 = 별칭2. 컬럼;
 */
/*
시나리오] 사원테이블에서 각 사원의 메니저아이디와 매니저 이름을 출력하시오
...
/*
using : join문에서 주로 사용하는  on 절을 대체할 수 있는 문장
...
using절에 사용된 컬럼은 양쪽의 테이블에 동시에 존재하는 컬럼이라는 것을 전제로 작성되기 때문에
굳이 별칭을 사용할 이유가 없기때문이다. 
즉, using은 테이블의 별칭 및 on절을 제거하여 좀 더 심플하게 join쿼리문을 작성할 수 있게 해준다.
*/
...
select first_name, hire_date, substr(hire_date, 1, 2)"연도1", to_char(hire_date, 'yyyy')"연도2"
from employees;
...
/*2005년에 입사했고, 50번 부서(shipping)에 근무하는 사원의 정보를 인출해야한다.*/

--join쿼리문 작성하기
----------------------------샘버전-----------------------------------------------
select
    employee_id, first_name, last_name, trim(to_char(salary, '$9999,000')), 
...
----------------------------내가 푼거---------------------------------------------
select * from locations where state_province = 'California' and city = 'South San Francisco';
select * from departments where location_id = 1500;    
...
from locations
    ...
    inner join employees using(department_id)
    inner join countries using(country_id)
where state_province = 'California' and city = 'South San Francisco' 
