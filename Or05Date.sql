...
파일명 : Or05Date.sql
날짜함수
설명 : 년, 월, 일, 시, 분, 초의 포맷으로 날짜형식을 지정하거나 날짜를 계산할 때 활용하는 함수들
*************************************************************************************/

/*
month_between() : 현재날짜와 기준날짜 사이의 개월 수를 반환한다.
    형식] month_between(현재날짜, 기준날짜[과거날짜]);
*/

--2020년 1월 1일부터 지금까지 지난 개월 수는?
...
from employees 
order by trunc(months_between(sysdate, hire_date)) asc;--방법1
--order by "근속개월수2" asc;--방법2
/*
select 결과를 정렬하기 위해 order by를 사용할 때 컬럼명은 위와같이 2가지 형태로 기술할 수 있다.
...
select last_day('22-02-03') from dual;--보통은 28일 출력
