...
설명 : 레코드를 조작할 때 사용하는 쿼리문, 앞에서 학습했던 select문을 비롯하여
    update(레코드수정), delete(레코드삭제), insert(레코드입력)가 있다.
***********************************/
...
/*
레코드 입력하기 : insert
    레코드 입력을 위한 쿼리문으로 문자형은 반드시 '로 감싸야하고, 숫자형은 '없이 기술한다.
    만약 숫자형에 '을 감싸는 경우에는 자동으로 반환되어 입력된다.
*/
...
--레코드입력2 : 컬럼 지정 없이 전체 컬럼을 대상으로 insert한다.
...
/*컬럼을 지정해서 insert 하는 경우 데이터를 삽입하지 않을 컬럼을 지정할 수 있다.
 아래의 경우 name컬럼에 null값이 삽입된다. */
insert into tb_sample (no, loc, manager)
    values(50, '제주', '동탁');
select * from tb_sample;

/*
지금까지의 작업(트랜잭션)을 그대로 유지하겠다는 명령으로 커밋을 수행하지 않으면 외부에서는 변경된
레코드를 확인할 수 없다. 여기서 말하는 외부란 Java/JSP와 같은 Oracle 이외의 프로그램을 말한다.
...
--전체레코드가 대상인 경우에는 where절을 생략하면 된다.
select * from tb_sample;
