...
파일명 : Or14View.sql
View(뷰)
설명 : View는 테이블로부터 생성된 가상의 테이블로 물리적으로는 존재하지 않고 논리적으로 존재하는 테이블이다.
**************************/
...
--4.데이터 사전에서 확인하기(생성시 사용한 쿼리문이 그대로 저장된다)
...
select * from view_employees;
/*
뷰생성시 기존테이블의 컬럼명을 변경해서 출력하고 싶다면 위와같이 변경할 컬럼명을 뷰이름 뒤에
소괄호로 명시해주면 된다. 
*/
...
*/
--select문 생성
/*생성시 연산식이 추가되어 논리적으로 컬럼이 추가되는 경우에는 반드시 별칭으로 컬럼명을 명시해야한다.
그렇지 않으면 에러가 발생한다.*/
select
    employee_id, last_name,
    trim(to_char((salary + (salary * nvl(commission_pct, 0))) * 12, '9999,000'))
    /*이렇게 논리적으로 만들어진 식은 무조건 별칭을 함께 만들어야한다.(문제에서 주어지지 않더라도)*/
from employees;
--뷰로 생성
