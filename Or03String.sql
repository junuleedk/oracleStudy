...
/*
concat(문자열1,문자열2)
: 문자열 1과 2를 서로 연결해서 출력하는 함수. 문자열 부분은 컬럼명을 사용할 수 있다.
...
-- => 위sql문을 concat()으로 번경해보자. (파이프보다 조금 불편하다)
...
*/
--step1 : 이름은 연결했지만 띄어쓰기가 안되서 가독성이 떨어짐
select
    concat(first_name, last_name), salary, department_id
from employees;

/*
step2 : 이름 사이에 스페이를 하나 추가한다. concat() 보다는 파이프가 편리하다.
또한 컬럼명은 as를 이용해서 별칭을 부여한다.
*/
select
...
/*
lpad(), rpad()
:문자열의 왼쪽, 오른쪽을 특정한 기호로 채울 떄 사용한다.
형식] lpad('문자열', '전체자리수', '채울문자열')
    => 전체자리수에서 문자열의 길이만큼을 제외한 나머지 좌(우)측 부분을 주어진 문자로 채워주는 함수
*/
--3번째 인자가 없으면 빈자리를 공백(스페이스)로 채우게된다. 
select
...
형식] trim([leading | trailing | both] 제거할 문자 from 컬럼)
    ...
    [주의1] 양쪽끝의 문자만 제거하고, 중간의 문자는 제거되지 않음
...
/*
시나리오]사원테이블의 first_name를 첫글자를 제외한 나머지 부분을 *로 마스킹 처리하는 쿼리문을 작성하시오
*/
--이름의 첫글자만 출력하기(인덱스 1부터 1글자를 잘라낸다)
select first_name, substr(first_name, 1, 1) from employees;
--이름을 10글자로 간주하여 나머지 ㅂ분을 *로 채워보기
select rpad(first_name, 10, '*')from employees;
--이름의 첫을자를 가져오고, 이름의 길이(length)를 얻어온다. 
--이를 톻해 *를 출력하면 컷글자를 제외한 남은 길이만큼 마스킹처리할 수 있다.
select first_name,
    rpad(substr(first_name, 1, 2), length(first_name), '*')"마스킹"
from employees;
/*
replace() : 문자열을 다른 문자열로 대체할 때 사용한다. 만약 공백으로 문자열을 대체한다면 문자열이
삭제되는 결과가 된다.
    형식]replace(컬럼명 or 문자열, '변경할 대상의 문자', '변경할 문자')
    
    ※trim(), ltrim(), rtrim()의 기능을 replace() 하나로 대체할 수 있으므로 trim()에 비해
    replace()가 훨씬 더 사용빈도가 높다.
*/
--문자열을 변경한다.
...
instr() : 문자열에서 특정문자가 위치한 인덱스값을 반환한다.
    ...
    형식2]instr(컬럼명, '찾을문자', '탐색을 시작할 인덱스', '몇번째 문자')
        : 탐색할 인덱스부터 문자를 찾는다. 단, 찾는 문자중 몇번째에 있는 문자인지 지정할 수 있다.
