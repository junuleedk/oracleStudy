...
제약조건 및 테이블목록 확인하기
    ...
    ※이와같이 제약조건이나 뷰, 프로시저등의 정보를 저장하고 있는 시스템 테이블을 "데이터 사전"이라고 한다.
*/
select * from tab;
...
/*무결성 제약조건 위배로 에러가 발생한다. PK로 지정된 컬럼 idx에는 중복된 값을 입력할 수 없다.*/
insert into tb_primary1 values(2, 'cityError', '오류발생');

insert into tb_primary1 (idx, user_id, user_name) values(3, 'white', '흰색');
/*PK로 지정된 컬럼에는 null값을 입력할 수 없다. 반드시 중복되지 않는 하나의 값을 입력해야한다.
따라서 에러가 발생한다.*/
insert into tb_primary1 (idx, user_id, user_name) values('', 'black', '검은색');
--앞에서 입력한 3개의 레코드가 조회된다. 
select * from tb_primary1;
/*쿼리문은 문제가 없으나 idx값이 이미 존재하는 2로 변경했으므로 제약조건 위배로 오류가 발생한다.*/
...
/*PK지정시 my_pkl이라고 이름을 지정했으므로 입력에러가 발생될 때 지정한 이름이
콘솔에 출력된다.*/
...
/*테이블을 생성한 후 alter문으로 제약조건을 부여한다. 제약명은 필요에 따라 생략이 가능하다.*/
...
create table tb_unique (
    idx number unique not null,--인라인방식
    name varchar2(30),
    telephone varchar2(20),
    nickname varchar2(30),
    unique(telephone, nickname)--아웃라인방식
);
...
/*
telephone과 nickname은 동일한 제약명으로 설정되었으므로 두개의 컬럼이 동시에 동일한 값을
가지는 경우가 아니라면 중복된 값이 허용된다. 
즉, 4번과 6번은 동일한 데이터로 인식되어 에러가 발생한다.
*/
...
insert into tb_foreign1 values(2, '홍길동', 'gildong');
...
create table tb_foreign2 (
    ...
    f_name varchar2(30),
    f_date date,
    /*tb foreign2 테이블의 f_id컬럼이 부모테이블인 tb_primary1의 idx컬럼을 참조하는
    외래키를 생성한다.*/
    foreign key (f_id) references tb_primary1 (idx)
);
select * from user_constraints;
/*
데이터 사전에 제약조건 확인을 위한 플래그
P : Primary key
...
[on delete casecade]
    ...
    형식] 
    컬럼명 자료형 refereneces 부모테이블(pk정렬)
        on delete cacasde;
[on delete set null]
     : 부모레코드 삭제시 자식레코드 같이 null로 변경된다. 
     형식] 
     컬럼명 자료형 references 부모테이블 (pk컬럼)
        on delete set null
        
※ 실무에서 스팸게시물을 남긴 회원과 그 게시글을 일괄적으로 삭제해야할 때 사용할 수 있는 옵션이다.
    단, 자식 테이블의 모든 레코드가 삭제되므로 사용에 주의해야한다.
*/
--user_id 를 기본키로 설정한 부모테이블
...
/*부모테이블에서 레코드를 삭제할 경우on delete cascade 옵션에 의해 
자식속까지 모든 레코드가 삭제된다. 만약 해당 옵션을 부여하지 않았다면
레도는 삭제되지 않고 오류가 발생하게된다.*/
-----------------------------------------
...
참조키 부분만  null값으로 변경된다. 따라서 더 이상 참조할 수 없는 레코드로 변경된다.*/
...
--update tb_not_null set pw = '1111', name = 'hong' where idx = 1;
/*오류발생. PK로 지정된 컬럼은 null값을 입력할 수 없다. insert 퀄리에서 컬럼을 명시하지 않으면 null값이
입력된다.*/
insert into tb_not_null (id, pw, name) values ('sa', '6666 ', '사오정');
select * from tb_not_null;

/*
default : insert시 아무런 값도 입력되지 않았을 때 자동으로 삽입되는 데이터를 지정한다.
*/
...
--default값이 입력된다는 것을 명시하여  디폴트 값을 입력한다.
...
/*
check : Domain(자료형) 무결성을 유지하기 위한 제약조건으로 컬럼에 잘못된 데이터가 입력되지 
        않도록 해주는 제약조건이다.
*/
